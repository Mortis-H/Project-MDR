...
#
#   6. 清除指定目錄的 relink 輸出：
#      ./clean_results.sh --type relink -p deterministic_kernels
#
#   7. 清除所有輸出：
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  $0 --type relink -p deterministic_kernels    # 清除特定目錄的 relink 輸出"
            echo "  $0 --type all -p ./my_kernels                # 清除指定目錄的所有輸出"
            echo "  $0 --type all --dry-run                      # 列出所有要清除的資料夾"
            echo "  $0 --type relink -p deterministic_kernels --no  # 互動確認"
            echo ""
            exit 0
            ;;
        *)
