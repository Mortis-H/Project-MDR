...
usage() {
    ...
    echo ""
    echo "範例:"
    echo "  $0 -p /path/to/run_success                       # 測試所有 kernel"
    echo "  $0 -p ./run_success -l 5                         # 只測試前 5 個 kernel"
    echo "  $0 -p ./run_success -k 0a32840dac54a492...       # 測試特定 kernel"
    echo "  $0 -p ./run_success -t 120                       # 設定超時時間為 120 秒"
    exit 1
}
...
if [ ! -d "$KERNEL_DIR" ]; then
    echo -e "${RED}錯誤：run_success 目錄不存在: $KERNEL_DIR${NC}"
    exit 1
fi
...
echo "  Run Success 目錄: $KERNEL_DIR"
...
if [ -n "$SPECIFIC_KERNEL" ]; then
    # 測試特定 kernel
    kernel_dir="${KERNEL_DIR}/${SPECIFIC_KERNEL}"
    if [ -d "$kernel_dir" ]; then
    ...
    # 測試所有或限定數量的 kernel
    for kernel_dir in "$KERNEL_DIR"/*/; do
        # 檢查是否達到限制
