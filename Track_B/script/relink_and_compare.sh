...
# 用法：./relink_and_compare.sh <原始執行檔> <host.o> <rebuilt.hsaco> [測試參數...]
...
# 函數：顯示使用方法
usage() {
    echo "用法: $0 <原始執行檔> <host.o> <rebuilt.hsaco> [測試參數...]"
    echo ""
    echo "參數說明:"
    echo "  <原始執行檔>    - 原始編譯的可執行檔路徑"
    echo "  <host.o>        - 主機端目標檔案路徑 (host-x86_64-unknown-linux-gnu.o)"
    echo "  <rebuilt.hsaco> - Pipeline 重建的 GPU 代碼檔案 (.hsaco)"
    echo "  [測試參數...]   - 執行檔所需的命令列參數（可選，多個參數用空格分隔）"
    echo ""
    echo "環境變數:"
    ...
    echo ""
    echo "範例:"
    echo "  $0 ./original_exe ./host.o ./rebuilt.hsaco 64"
    echo "  $0 ./original_exe ./host.o ./rebuilt.hsaco 128 extra_arg"
    echo ""
    echo "  # 自訂輸出目錄"
    echo "  OUTPUT_BASE_DIR=/tmp/test_results $0 ./exe ./host.o ./hsaco 64"
    echo ""
    echo "  # 自訂超時時間"
    echo "  EXECUTION_TIMEOUT=120 $0 ./exe ./host.o ./hsaco 64"
    exit 1
}
